 ...
 * @param {string} emailData.replyToName - Reply-to name (optional)
 * @returns {Promise<Object>} Result object with success status
 */
export async function sendEmail(env, emailData) {
  try {
    const { to, subject, html, text, from, fromName, replyTo, replyToName } =
      emailData;
    ...
    if (replyTo) {
      // Format Reply-To with name if provided, otherwise just use email
    ...
    <body>
        ...
        ${
          jobTitle && companyName
            ? `<p><strong>Sobre:</strong> ${jobTitle} - ${companyName}</p>`
            : ""
        }
        ...
        ${
          jobTitle && companyName
            ? `
        <p><strong>Posición:</strong> ${jobTitle}<br>
        <strong>Empresa:</strong> ${companyName}</p>
        `
            : ""
        }
        ...
        <p>Para responder a este mensaje, simplemente haz clic en "Responder" en tu cliente de correo.</p>

        <p>Este mensaje se envió a través de nuestro sistema de mensajería seguro. Tus datos de contacto permanecen privados y protegidos.</p>

        <hr>

        <p><strong>Acerca de este mensaje:</strong><br>
        Este mensaje fue enviado a través de Empleo Safari, una plataforma que conecta a candidatos con oportunidades laborales.
        Tu dirección de correo real está protegida por nuestro sistema de relay.</p>

        <p>Para responder, simplemente usa el botón "Responder" de tu cliente de correo. 
        Tu respuesta será entregada de forma segura a ${fromName}.</p>
