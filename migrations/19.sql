-- Migration 19: Remove unique constraint for (user_id, job_id) in user_job_applications table

-- SQLite doesn't support dropping constraints directly, so we need to recreate the table
-- without the UNIQUE(user_id, job_id) constraint
...
CREATE TABLE user_job_applications_new (
  ...
  -- Note: Removed UNIQUE(user_id, job_id) constraint
);

-- Step 2: Copy data from old table to new table
INSERT INTO user_job_applications_new 
SELECT id, user_id, job_id, status, credits_spent, unlocked_at, applied_at, response_at, notes, created_at, updated_at, contact_id
...
-- Step 5: Recreate indexes (excluding the old unique constraint)
...
-- Step 6: Recreate the unique index for (user_id, job_id, contact_id) if it was created in migration 18
